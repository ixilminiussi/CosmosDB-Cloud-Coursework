...
def main(req: func.HttpRequest, writer: func.Out[func.Document]) -> func.HttpResponse:
    ...
    #find appropriate prompt ID, by retrieving the prompt with the highest ID, and adding one
    ...
    if (prompts == []):
        ...
        id = int(prompts[0].get("id")) + 1

    prompt = {"id" : str(id), "username": username, "text" : prompt}
